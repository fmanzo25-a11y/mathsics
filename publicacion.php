...
try {
    ...
    $stmtPost = $conn->prepare(
        "SELECT p.*, u.id as id_autor, u.nombre as nombre_autor, u.foto_de_perfil as foto_autor 
         FROM posts p 
    ...
    $stmtCom = $conn->prepare(
        "SELECT c.*, u.id as id_autor_comentario, u.nombre, u.foto_de_perfil 
         FROM comentarios c 
            ...
            <div class="p-6 sm:p-8">
                <div class="flex items-center gap-4 mb-5">
                    <a href="usuario.php?id=<?php echo $post['id_autor']; ?>" class="cursor-pointer">
                        <img src="<?php echo htmlspecialchars($post['foto_autor'] ?? 'images/sinfoto.jpeg'); ?>" alt="Foto de perfil" class="w-14 h-14 rounded-full object-cover border-2 border-white shadow-md hover:border-blue-400 transition">
                    </a>
                    <div>
                        <a href="usuario.php?id=<?php echo $post['id_autor']; ?>" class="font-bold text-lg text-gray-800 hover:text-blue-600 hover:underline"><?php echo htmlspecialchars($post['nombre_autor']); ?></a>
                ...
                ?>
                    ...
                    ?>
                        <div class="flex items-start gap-4">
                            <a href="usuario.php?id=<?php echo $com['id_autor_comentario']; ?>" class="cursor-pointer">
                                <img src="<?php echo htmlspecialchars($com['foto_de_perfil'] ?? 'images/sinfoto.jpeg'); ?>" alt="Foto de perfil" class="w-10 h-10 rounded-full object-cover mt-1 shadow-sm border-2 border-transparent hover:border-indigo-400 transition">
                            </a>
                            <div class="flex-1 bg-gray-100 rounded-xl p-4">
                                <div class="flex justify-between items-center mb-1">
                                    <a href="usuario.php?id=<?php echo $com['id_autor_comentario']; ?>" class="font-semibold text-indigo-700 hover:underline"><?php echo htmlspecialchars($com['nombre']); ?></a>
