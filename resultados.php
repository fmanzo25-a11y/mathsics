...
foreach ($desafiosActivos as $desafio) {
    ...
    // Desafío tipo "leccion" o "aciertos"
    if ($desafio['tipo'] === 'aciertos' || $desafio['tipo'] === 'leccion') {
        $incremento = ($desafio['tipo'] === 'aciertos') ? $aciertos : 1;
        $stmtProgreso = $conn->prepare("UPDATE usuario_desafios SET progreso = progreso + :incremento WHERE id = :id");
        $stmtProgreso->execute(['incremento' => $incremento, 'id' => $desafio['id']]);
        $progresoActualizado = true;
        $nuevoProgreso += $incremento;
    }
    // Desafío tipo "xp"
    else if ($desafio['tipo'] === 'xp' && $xpGanada > 0) {
        $stmtProgreso = $conn->prepare("UPDATE usuario_desafios SET progreso = progreso + :xpGanada WHERE id = :id");
        $stmtProgreso->execute(['xpGanada' => $xpGanada, 'id' => $desafio['id']]);
        $progresoActualizado = true;
        $nuevoProgreso += $xpGanada;
    }
...
<body class="min-h-screen flex flex-col items-center justify-center p-4 text-slate-700 relative overflow-x-hidden">

    <!-- Background Blobs -->
    <div class="fixed inset-0 z-[-1] overflow-hidden pointer-events-none">
        <div class="absolute top-[0%] left-[0%] w-96 h-96 bg-blue-300 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob"></div>
        <div class="absolute top-[30%] right-[0%] w-96 h-96 bg-pink-300 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob animation-delay-2000"></div>
    ...
    <main class="relative glass-panel rounded-3xl shadow-xl w-full max-w-2xl text-center p-6 sm:p-8 md:p-10 animate-slide-in 
    ...
    <audio id="level-up-sound" src="sonidos/xp.mp3" preload="auto"></audio>
